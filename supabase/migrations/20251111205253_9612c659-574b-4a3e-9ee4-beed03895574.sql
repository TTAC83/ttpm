-- Fix variable name collision in get_line_full_data function
-- The parameter p_line_id conflicts with positions.line_id (aliased as p.line_id)
-- This causes the WHERE clause to fail, especially for solutions_lines

DROP FUNCTION IF EXISTS public.get_line_full_data(UUID, TEXT);

CREATE OR REPLACE FUNCTION public.get_line_full_data(
  p_input_line_id UUID,
  p_table_name TEXT
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  IF p_table_name = 'lines' THEN
    ...
    FROM lines l
    WHERE l.id = p_input_line_id;
  ELSE
    SELECT to_jsonb(sl.*) INTO v_line
    FROM solutions_lines sl
    WHERE sl.id = p_input_line_id;
  END IF;
  ...
  WHERE (
    ...
    OR
    (p_table_name = 'solutions_lines' AND p.solutions_line_id = p_input_line_id)
  );
...
COMMENT ON FUNCTION public.get_line_full_data(UUID, TEXT) IS 
'Optimized function to fetch all line data including positions, equipment, cameras, and related tables in a single query. 
Reduces N+1 query problems. Fixed variable name collision between parameter and table column.';
