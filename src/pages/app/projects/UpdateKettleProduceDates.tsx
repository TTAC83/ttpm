...
import { updateKettleProduceDates } from '@/utils/updateKettleProduceDates';

export const UpdateKettleProduceDates = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const updateResults = await updateKettleProduceDates();
      ...
      toast({
        title: "Error",
        description: "Failed to update Kettle Produce project dates",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Update Kettle Produce Project Dates</h1>
        <p className="text-muted-foreground">
          Update task dates for the Kettle Produce project with predefined data
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Kettle Produce Date Update</CardTitle>
          <CardDescription>
            This will update all task dates for the Kettle Produce project with the latest schedule data.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            ...
            className="w-full"
          >
            {isUpdating ? 'Updating...' : 'Update Kettle Produce Dates'}
          </Button>
