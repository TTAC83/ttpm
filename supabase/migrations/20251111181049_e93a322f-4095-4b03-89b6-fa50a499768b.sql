-- Create optimized function to fetch all line data in a single query
CREATE OR REPLACE FUNCTION public.get_line_full_data(
  p_line_id UUID,
  p_table_name TEXT -- 'lines' or 'solutions_lines'
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
Reduces N+1 query problems that were causing timeout errors.';
