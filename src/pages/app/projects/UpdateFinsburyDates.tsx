...
import { updateFinsburyDates } from '@/utils/updateFinsburyDates';

export const UpdateFinsburyDates = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const updateResults = await updateFinsburyDates();
      ...
      toast({
        title: "Error",
        description: "Failed to update Finsbury project dates",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Update Finsbury Project Dates</h1>
        <p className="text-muted-foreground">
          Update task dates for the Finsbury project with predefined data
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Finsbury Date Update</CardTitle>
          <CardDescription>
            This will update all task dates for the Finsbury project with the latest schedule data.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            ...
            className="w-full"
          >
            {isUpdating ? 'Updating...' : 'Update Finsbury Dates'}
          </Button>
