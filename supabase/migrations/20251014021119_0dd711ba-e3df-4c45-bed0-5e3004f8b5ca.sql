-- Create project_hardware_requirements table
CREATE TABLE IF NOT EXISTS public.project_hardware_requirements (
...
-- Enable RLS on project_hardware_requirements
ALTER TABLE public.project_hardware_requirements ENABLE ROW LEVEL SECURITY;

-- RLS policies for project_hardware_requirements
CREATE POLICY "hardware_req_internal_all"
  ON public.project_hardware_requirements
  FOR ALL
  USING (is_internal())
  WITH CHECK (is_internal());

CREATE POLICY "hardware_req_external_select"
  ON public.project_hardware_requirements
  FOR SELECT
  USING (
    (project_id IN (
...
CREATE POLICY "hardware_req_external_insert"
  ON public.project_hardware_requirements
  FOR INSERT
  WITH CHECK (
    EXISTS (
    ...
    EXISTS (
      ...
      WHERE bc.id = project_hardware_requirements.bau_customer_id
      AND bc.company_id = user_company_id()
    )
  );

CREATE POLICY "hardware_req_external_update"
  ON public.project_hardware_requirements
  FOR UPDATE
  USING (
    EXISTS (
...
CREATE POLICY "hardware_req_external_delete"
  ON public.project_hardware_requirements
  FOR DELETE
  USING (
    EXISTS (
