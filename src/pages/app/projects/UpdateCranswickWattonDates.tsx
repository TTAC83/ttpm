...
import { updateCranswickWattonDates } from '@/utils/updateCranswickWattonDates';

export const UpdateCranswickWattonDates = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const updateResults = await updateCranswickWattonDates();
      ...
      toast({
        title: "Error",
        description: "Failed to update Cranswick Watton project dates",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Update Cranswick Watton Project Dates</h1>
        <p className="text-muted-foreground">
          Update task dates for the Cranswick Watton project with predefined data
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Cranswick Watton Date Update</CardTitle>
          <CardDescription>
            This will update all task dates for the Cranswick Watton project with the latest schedule data.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            ...
            className="w-full"
          >
            {isUpdating ? 'Updating...' : 'Update Cranswick Watton Dates'}
          </Button>
