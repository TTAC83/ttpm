...
interface SharedProductGapsTabProps {
  projectId: string;
  projectType: 'implementation' | 'solutions';
}

export function SharedProductGapsTab({ projectId, projectType }: SharedProductGapsTabProps) {
  ...
  const { data: productGaps = [], isLoading } = useQuery({
    queryKey: ['project-product-gaps', projectId, projectType],
    queryFn: () => productGapsService.getProjectProductGaps(projectId, projectType),
  });
