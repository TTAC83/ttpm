...
import { updateMBCDates } from '@/utils/updateMBCDates';

export const UpdateMBCDates = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const updateResults = await updateMBCDates();
      ...
      toast({
        title: "Error",
        description: "Failed to update MBC project dates",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Update MBC Project Dates</h1>
        <p className="text-muted-foreground">
          Update task dates for the MBC project with predefined data
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>MBC Date Update</CardTitle>
          <CardDescription>
            This will update all task dates for the MBC project with the latest schedule data.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            ...
            className="w-full"
          >
            {isUpdating ? 'Updating...' : 'Update MBC Dates'}
          </Button>
