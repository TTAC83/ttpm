      ...
      const positions: Position[] = (result.positions || []).map((pos: any) => ({
        ...
        equipment: (pos.equipment || []).map((eq: any) => ({
          ...
          cameras: (eq.cameras || []).map((cam: any) => ({
            ...
            light_required: cam.light_required === true ? true : (cam.light_id ? false : null),
            light_id: cam.light_id || undefined,
            light_notes: cam.light_notes || "",
            plc_attached: cam.plc_attached === true ? true : (cam.plc_master_id ? false : null),
            plc_master_id: cam.plc_master_id || undefined,
            hmi_required: cam.hmi_required === true ? true : (cam.hmi_master_id ? false : null),
        ...
        for (const camera of eq.cameras) {
          const { data: cameraData, error: cameraError } = await supabase
            ...
            .insert({
              ...
              light_id: camera.light_id && camera.light_id !== 'non-standard' ? camera.light_id : null,
              light_notes: camera.light_notes || null,
              plc_attached: camera.plc_attached ?? null,
              plc_master_id: camera.plc_master_id && camera.plc_master_id !== 'non-standard' ? camera.plc_master_id : null,
              hmi_required: camera.hmi_required ?? null,
              hmi_master_id: camera.hmi_master_id && camera.hmi_master_id !== 'non-standard' ? camera.hmi_master_id : null,
