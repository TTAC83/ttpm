...
import { updateButternutBoxDates } from '@/utils/updateButternutBoxDates';

export const UpdateButternutBoxDates = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const updateResults = await updateButternutBoxDates();
      ...
      toast({
        title: "Error",
        description: "Failed to update Butternut Box project dates",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Update Butternut Box Project Dates</h1>
        <p className="text-muted-foreground">
          Update task dates for the Butternut Box project with predefined data
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Butternut Box Date Update</CardTitle>
          <CardDescription>
            This will update all task dates for the Butternut Box project with the latest schedule data.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            ...
            className="w-full"
          >
            {isUpdating ? 'Updating...' : 'Update Butternut Box Dates'}
          </Button>
