...
import { updateButlersFarmhouseDates } from '@/utils/updateButlersFarmhouseDates';

export const UpdateButlersFarmhouseDates = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const updateResults = await updateButlersFarmhouseDates();
      ...
      toast({
        title: "Error",
        description: "Failed to update Butlers Farmhouse project dates",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Update Butlers Farmhouse Project Dates</h1>
        <p className="text-muted-foreground">
          Update task dates for the Butlers Farmhouse project with predefined data
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Butlers Farmhouse Date Update</CardTitle>
          <CardDescription>
            This will update all task dates for the Butlers Farmhouse project with the latest schedule data.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            ...
            className="w-full"
          >
            {isUpdating ? 'Updating...' : 'Update Butlers Farmhouse Dates'}
          </Button>
