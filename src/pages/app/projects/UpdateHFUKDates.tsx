...
import { updateHFUKDates } from '@/utils/updateHFUKDates';

export const UpdateHFUKDates = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const updateResults = await updateHFUKDates();
      ...
      toast({
        title: "Error",
        description: "Failed to update HFUK project dates",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Update HFUK Project Dates</h1>
        <p className="text-muted-foreground">
          Update task dates for the HFUK project with predefined data
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>HFUK Date Update</CardTitle>
          <CardDescription>
            This will update all task dates for the HFUK project with the latest schedule data.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            ...
            className="w-full"
          >
            {isUpdating ? 'Updating...' : 'Update HFUK Dates'}
          </Button>
