-- BAU Weekly Review System Migration
-- Forward SQL migration (idempotent)

-- 1) Enums
CREATE TYPE IF NOT EXISTS bau_health_simple AS ENUM ('green','red');
...
CREATE POLICY IF NOT EXISTS read_uploads ON bau_weekly_uploads
...
CREATE POLICY IF NOT EXISTS read_metrics ON bau_weekly_metrics
FOR SELECT USING ( is_internal() OR EXISTS (
  SELECT 1 FROM bau_customers bc WHERE bc.id = bau_weekly_metrics.bau_customer_id AND bc.company_id = user_company_id()
));

CREATE POLICY IF NOT EXISTS read_reviews ON bau_weekly_reviews
FOR SELECT USING ( is_internal() OR EXISTS (
  SELECT 1 FROM bau_customers bc WHERE bc.id = bau_weekly_reviews.bau_customer_id AND bc.company_id = user_company_id()
));

CREATE POLICY IF NOT EXISTS read_aliases ON bau_customer_aliases
FOR SELECT USING ( is_internal() );

CREATE POLICY IF NOT EXISTS read_catalog ON bau_metric_catalog
FOR SELECT USING ( true );

-- Write rules: internal may insert/update; external can write reviews for their company only
CREATE POLICY IF NOT EXISTS write_metrics_internal ON bau_weekly_metrics
FOR ALL USING ( is_internal() ) WITH CHECK ( is_internal() );

CREATE POLICY IF NOT EXISTS write_reviews_internal ON bau_weekly_reviews
...
CREATE POLICY IF NOT EXISTS write_uploads_internal ON bau_weekly_uploads
FOR ALL USING ( is_internal() ) WITH CHECK ( is_internal() );

CREATE POLICY IF NOT EXISTS write_aliases_internal ON bau_customer_aliases
FOR ALL USING ( is_internal() ) WITH CHECK ( is_internal() );

CREATE POLICY IF NOT EXISTS write_catalog_internal ON bau_metric_catalog
FOR ALL USING ( is_internal() ) WITH CHECK ( is_internal() );
...
CREATE POLICY IF NOT EXISTS storage_bau_weekly_read
ON storage.objects FOR SELECT
USING ( bucket_id = 'bau-weekly-uploads' AND is_internal() );

CREATE POLICY IF NOT EXISTS storage_bau_weekly_write
ON storage.objects FOR INSERT
WITH CHECK ( bucket_id = 'bau-weekly-uploads' AND is_internal() );

CREATE POLICY IF NOT EXISTS storage_bau_weekly_update
