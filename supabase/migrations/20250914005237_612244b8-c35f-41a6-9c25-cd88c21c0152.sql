...
-- 3) VIEWS - Corrected based on actual schema
CREATE OR REPLACE VIEW v_distinct_customers AS
SELECT DISTINCT customer_name AS customer
FROM (
  SELECT name AS customer_name FROM projects WHERE name IS NOT NULL
  UNION ALL
  SELECT company_name AS customer_name FROM solutions_projects WHERE company_name IS NOT NULL
) p
