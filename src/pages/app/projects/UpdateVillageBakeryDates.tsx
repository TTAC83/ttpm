...
import { updateVillageBakeryDates } from '@/utils/updateVillageBakeryDates';

export const UpdateVillageBakeryDates = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const updateResults = await updateVillageBakeryDates();
      ...
      toast({
        title: "Error",
        description: "Failed to update Village Bakery project dates",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Update Village Bakery Project Dates</h1>
        <p className="text-muted-foreground">
          Update task dates for the Village Bakery project with predefined data
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Village Bakery Date Update</CardTitle>
          <CardDescription>
            This will update all task dates for the Village Bakery project with the latest schedule data.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            ...
            className="w-full"
          >
            {isUpdating ? 'Updating...' : 'Update Village Bakery Dates'}
          </Button>
